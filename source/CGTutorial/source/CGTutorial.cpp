...
int main(void)
{
    ...
    // Die folgenden vier Zeilen sind n�tig auf dem Mac
    // Au�erdem m�ssen die zu ladenden Dateien bei der aktuellen Projektkonfiguration
    ...
    // Ab hier kann man alles wunderbar in eine Klasse oder Utility-Funktion auslagern, z. B.:
    // GLuint arraySize; // wuerde vertices.size() in glDrawArrays(...); ersetzen
    // GLuint VertexArrayIDTeapot = readObject("teapot.obj", &arraySize);
    // In main dann: glBindVertexArray(VertexArrayIDTeapot); glDrawArrays(GL_TRIANGLES, 0, arraySize);
    // Textur mueeste weiterhin separat behandelt werden...
    std::vector<glm::vec3> vertices;
    std::vector<glm::vec2> uvs;
    std::vector<glm::vec3> normals;

    bool res = loadOBJ("/Users/marvinkruger/CG/source/CGTutorial/resources/teapot.obj", vertices, uvs, normals);
